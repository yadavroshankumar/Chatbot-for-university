...
root.title("VIT - Assistant")
root.resizable(0,0)
text = tkinter.StringVar()
conversation=["        Welcome to VIT.       \n".center(120)]
text.set("\n".join(conversation))
...
label = tkinter.Label(root, textvariable=text,height=30,width=60,justify=tkinter.LEFT,
                     anchor='nw',font={"family":"Arial Black", "size":15})
...
def ReadText():
    ...
    conversation.append("VITBOT: " + reply)
    ...
    if 'image' in [list(i.keys())[1] for i in ast.literal_eval(x.text)]:
        ...
        conversation.append("VITBOT: " + ast.literal_eval(x.text)[1]["image"].replace("\\",""))
